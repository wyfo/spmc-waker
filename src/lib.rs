...
impl<const SYNC: bool, const CACHED: bool> SpmcWaker<SYNC, CACHED> {
    ...
    fn wake_unsync<R>(
        ...
        #[cfg(not(miri))]
        return self.unregister_and_wake(vtable, |vt| self.vtable.store(vt, Release), wake);
        #[cfg(miri)] // See https://github.com/rust-lang/miri/issues/5104
        return self.unregister_and_wake(vtable, |vt| self.vtable.store(vt, SeqCst), wake); // !ORDERING
    }
